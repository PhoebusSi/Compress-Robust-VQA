...
#layer:language,r_layer:visual,x_layer:fusion
_lxmert_names = {
    "E": lambda ptl, l: f"{ptl}.embeddings.word_embeddings",
    "VV": lambda ptl, l: f"{ptl}.encoder.visn_fc.visn_fc",
    "VB": lambda ptl, l: f"{ptl}.encoder.visn_fc.box_fc",
    "lK": lambda ptl, l: f"{ptl}.encoder.layer.{l}.attention.self.key",
    "lQ": lambda ptl, l: f"{ptl}.encoder.layer.{l}.attention.self.query",
    "lV": lambda ptl, l: f"{ptl}.encoder.layer.{l}.attention.self.value",
    "lAO": lambda ptl, l: f"{ptl}.encoder.layer.{l}.attention.output.dense",
    "lI": lambda ptl, l: f"{ptl}.encoder.layer.{l}.intermediate.dense",
    "lO": lambda ptl, l: f"{ptl}.encoder.layer.{l}.output.dense",
    "vK": lambda ptl, l: f"{ptl}.encoder.r_layers.{l}.attention.self.key",
    "vQ": lambda ptl, l: f"{ptl}.encoder.r_layers.{l}.attention.self.query",
    "vV": lambda ptl, l: f"{ptl}.encoder.r_layers.{l}.attention.self.value",
    "vAO": lambda ptl, l: f"{ptl}.encoder.r_layers.{l}.attention.output.dense",
    "vI": lambda ptl, l: f"{ptl}.encoder.r_layers.{l}.intermediate.dense",
    "vO": lambda ptl, l: f"{ptl}.encoder.r_layers.{l}.output.dense",
    "vlVK": lambda ptl, l: f"{ptl}.encoder.x_layers.{l}.visual_attention.att.key",
    "vlVQ": lambda ptl, l: f"{ptl}.encoder.x_layers.{l}.visual_attention.att.query",
    "vlVV": lambda ptl, l: f"{ptl}.encoder.x_layers.{l}.visual_attention.att.value",
    "vlVAO": lambda ptl, l: f"{ptl}.encoder.x_layers.{l}.visual_attention.output.dense",
    "vlLaK": lambda ptl, l: f"{ptl}.encoder.x_layers.{l}.lang_self_att.self.key",
    "vlLaQ": lambda ptl, l: f"{ptl}.encoder.x_layers.{l}.lang_self_att.self.query",
    "vlLaV": lambda ptl, l: f"{ptl}.encoder.x_layers.{l}.lang_self_att.self.value",
    "vlLaAO": lambda ptl, l: f"{ptl}.encoder.x_layers.{l}.lang_self_att.output.dense",
    "vlVaK": lambda ptl, l: f"{ptl}.encoder.x_layers.{l}.visn_self_att.self.key",
    "vlVaQ": lambda ptl, l: f"{ptl}.encoder.x_layers.{l}.visn_self_att.self.query",
    "vlVaV": lambda ptl, l: f"{ptl}.encoder.x_layers.{l}.visn_self_att.self.value",
    "vlVaAO": lambda ptl, l: f"{ptl}.encoder.x_layers.{l}.visn_self_att.output.dense",
    "vlLi": lambda ptl, l: f"{ptl}.encoder.x_layers.{l}.lang_inter.dense",
    "vlLo": lambda ptl, l: f"{ptl}.encoder.x_layers.{l}.lang_output.dense",
    "vlVi": lambda ptl, l: f"{ptl}.encoder.x_layers.{l}.visn_inter.dense",
    "vlVo": lambda ptl, l: f"{ptl}.encoder.x_layers.{l}.visn_output.dense",
    "P": lambda ptl, l: f"{ptl}.pooler.dense",
}
...
def chain_module_names(which_ptl, layer_idices, abbres):
    ...
    """
    _plt_names = _lxmert_names
    names = set()
    mapping = { 'E':'Lang', 'VV':'Vis', 'VB':'Vis', 'lK':'Lang', 'lQ':'Lang', 'lV':'Lang', 'lAO':'Lang', 'lI':'Lang', 'lO':'Lang', 'vK':'Vis', 'vQ':'Vis', 'vV':'Vis', 'vAO':'Vis', 'vI':'Vis', 'vO':'Vis', 'vlVK':'Fus', 'vlVQ':'Fus', 'vlVV':'Fus', 'vlVAO':'Fus', 'vlLaK':'Fus', 'vlLaQ':'Fus', 'vlLaV':'Fus', 'vlLaAO':'Fus', 'vlVaK':'Fus', 'vlVaQ':'Fus', 'vlVaV':'Fus', 'vlVaAO':'Fus', 'vlLi':'Fus', 'vlLo':'Fus', 'vlVi':'Fus', 'vlVo':'Fus', 'P':'P'}
    name_in_module = {}
    name_in_modal = {}
    name_in_layer = {}
    for abbre in abbres:
        for l in layer_idices:
            ...
            name_in_module[_plt_names[abbre](which_ptl, l)] = abbre
            name_in_modal[_plt_names[abbre](which_ptl, l)] = mapping[abbre]
            if mapping[abbre] in ['P','E','VV','VB']:
                name_in_layer[_plt_names[abbre](which_ptl, l)] = mapping[abbre]
            else:
                name_in_layer[_plt_names[abbre](which_ptl, l)] = mapping[abbre]+'_'+str(l) 


    return names, name_in_modal, name_in_module, name_in_layer
...
class MaskedLinear1(MaskedLinearX):
    ...
    def get_masks(self):
        M_w = self.threshold_fn(self.weight_mask , self.threshold.cuda())
        M_w = reshape_mask_for_sp(M_w, self.structured_mask_expanding, name="weight")

        if self.mask_biases:
            M_b = self.threshold_fn(self.bias_mask, self.threshold.cuda())
...
class Masker(object):
    def __init__(
        ...
        self.names_tobe_masked = None #names_tobe_masked
        self.name_in_module = None #name_in_module
        self.name_of_masker= None #name_of_masker
    ...
    def patch_modules(self, model, names_tobe_masked, name_of_masker="MaskedLinear1"):
        ...
        self.logger.info("Check the trainable status.")
        for _name, param in model.named_parameters():
            if param.requires_grad is True:
                self.logger.info(f"\t {_name} is trainable.")
            else:
                self.logger.info(f"\t {_name} is not trainable.")
    ...
    def replace(self, m, root_name, names_tobe_masked, name_in_module, masked_linear_cls):
        zerorate_dict = self.hpmodel.zerorate_dict
        # print('in maskers in zerorate_dict', zerorate_dict)
        for attr_str in dir(m):
            target_attr = getattr(m, attr_str)
            if isinstance(target_attr, nn.Module):
                name = root_name + "." + attr_str if len(root_name) > 0 else attr_str
                if hasattr(target_attr, 'weight') and not ('classifier' in name or 'lm_head' in name):
                    if target_attr.weight is not None:
                        target_attr.weight.requires_grad = False
                if hasattr(target_attr, 'bias') and not ('classifier' in name or 'lm_head' in name): 
                    if target_attr.bias is not None:
                        target_attr.bias.requires_grad = False

            if type(target_attr) == nn.Linear or type(target_attr) == nn.Embedding:
                masked = False

                if name in names_tobe_masked:
                    module = name_in_module[name]
                    ...
                    masked_linear = masked_linear_cls(
                        ...
                        init_sparsity=zerorate_dict[module], 
