...
def parse_args():
    ...
    parser.add_argument('--input', type=str, default='saved_models/xxx.json')
    parser.add_argument('--name', type=str, default='test')
    parser.add_argument('--dataroot', type=str, default='/xxxx/')
...
if __name__ == '__main__':
	...
	#        myfile.seek(0)
	#        predictions = sorted(json.load(myfile), key=lambda x: x['question_id'])
	...
	for pred, anno in zip(predictions, annotations):
		if pred['question_id'] == anno['question_id']:
			x=x+1
			if x<100:
				if pred['answer'] not in anno['answers_word']:
					print(pred['question_id'],":","pred:",pred['answer'],"anno",anno['answers_word'])
