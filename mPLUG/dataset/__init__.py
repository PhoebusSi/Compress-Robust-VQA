...
def vqa_bias_collate_fn(batch):
    image_list, question_list, answer_list, weight_list, n, bias_list = [], [], [], [], [], []
    for image, question, answer, weights, bias in batch:
        image_list.append(image)
        question_list.append(question)
        weight_list += weights       
        answer_list += answer
        n.append(len(answer))
        bias_list += bias
    return torch.stack(image_list,dim=0), question_list, answer_list, torch.Tensor(weight_list), n, torch.Tensor(bias_list)
