...
from .visualbert_file_utils import (
    ModelOutput,
...
from .configuration_visualbert import visualBERTConfig as VisualBertConfig
...
visualBERT_PRETRAINED_MODEL_ARCHIVE_MAP = {
...
class VisualBertLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = 0 #config.chunk_size_feed_forward
        self.seq_len_dim = 1
        # self.feed_forward_chunk = 0
...
class VisualBertEncoder(nn.Module):
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            # print("lyer_outputs", layer_outputs.shape)
...
class VisualBertForMultipleChoice(VisualBertPreTrainedModel):
    def __init__(self, config):
        ...
        self.cls = SimpleClassifier(in_dim=config.hidden_size, hid_dim=2 * config.hidden_size, out_dim=config.ans_num,
    ...
    def forward(
        ...
        logits = self.cls(pooled_output_drop)
        # print("logits", logits.shape)
        reshaped_logits = logits #logits.view(-1, num_choices)
        ...
        if not return_dict:
            # print("not_retrun_dict")
            #为了和lxmert统一一下outputs前三个分别是：loss，logits，pool_out，把pooled_output改成了last_hidden_state的数值了。
            output = (reshaped_logits,) + pooled_output #outputs[2:]
            # print("output")
            # print("loss", loss)
            # print("output, dic2", output)
