...
class LearnedMixin(DebiasLossFn):
    def __init__(self, w, smooth=True, smooth_init=-1, constant_smooth=0.0):
        ...
        if self.smooth:
            self.smooth_param = torch.nn.Parameter(
              torch.from_numpy(np.full((1,), smooth_init, dtype=np.float32)))
            print("INIT_smooth", self.smooth_param)
        else:
            self.smooth_param = None

    def forward(self, hidden, logits, bias, labels, device):
        ...
        sum_prob= (log_prob * labels + (1 - labels) * log_one_minus_prob).sum(1)

        sum_prob = torch.where(torch.isnan(sum_prob), torch.full_like(sum_prob, 0), sum_prob)

        loss = -(sum_prob.mean(0))
        ##loss = -(log_prob * labels + (1 - labels) * log_one_minus_prob).sum(1).mean(0)
        ...
        return loss + self.w * entropy#.to(device)
