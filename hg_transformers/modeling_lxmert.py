...
LXMERT_PRETRAINED_MODEL_ARCHIVE_MAP = [
    "unc-nlp/lxmert-base-uncased",
]
...
@add_start_docstrings(
    """Lxmert Model with a multiple choice classification head on top (a linear layer on top of
    the pooled output and a softmax) e.g. for RocStories/SWAG tasks. """,
)
class LxmertForMultipleChoice(LxmertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config 
        #self.num_labels = config.num_labels #不需要，我们不是选出3个，还是1个，只不过是多标签而已
        self.lxmert = LxmertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        #self.normal = nn.BatchNorm1d(config.hidden_size,affine=False)
        #self.classifier = nn.Linear(config.hidden_size, config.ans_num)
        self.classifier = SimpleClassifier(in_dim=config.hidden_size, hid_dim=2 * config.hidden_size, out_dim=config.ans_num,
                                          dropout=0.5, norm='weight', act='ReLU')
        #self.loss = self.instance_bce_with_logits()
    ...
    Returns:
        :obj:`tuple(torch.FloatTensor)` comprising various elements depending on the configuration (:class:`~transformers.LxmertConfig`) and inputs:
    ...
    Examples::
        ...
        from transformers import LxmertTokenizer, LxmertForMultipleChoice
        import torch

        tokenizer = LxmertTokenizer.from_pretrained('lxmert-base-uncased')
        model = LxmertForMultipleChoice.from_pretrained('lxmert-base-uncased')
        ...
        """
        #print("im in modeling_lxmert and before conput outputs.")
        outputs = self.lxmert(
            input_ids=input_ids,
        ...
        #pooled_output = self.normal(pooled_output) #根据SSL中也加一个normalization

        #pooled_output = self.dropout(pooled_output) #去掉这个dropout试下，之前都要去掉的
        logits = self.classifier(pooled_output) #两层ffn试一下！
        #reshaped_logits = logits.view(-1, num_choices)

        #outputs = (reshaped_logits,) + outputs[2:]  # add hidden states and attention if they are here
        #print("im in modeling_lxmert and after comput outputs and before labels.")
        if labels is not None:
            #loss_fct = CrossEntropyLoss()
            #loss = loss_fct(reshaped_logits, labels)
            #outputs = (loss,) + outputs
            loss = self.instance_bce_with_logits(logits, labels)
        #print("im in modeling_lxmert and after labels.")
        #assert 1==2

        return loss, logits, pooled_output #outputs  # (loss), reshaped_logits, (hidden_states), (attentions)




class LxmertForLMH(LxmertPreTrainedModel): #useless, 目前没有使用，而是在mask_trainer_VQA中使用了。
    def __init__(self, config):
        super().__init__(config)
        self.config = config 
        #self.num_labels = config.num_labels #不需要，我们不是选出3个，还是1个，只不过是多标签而已
    ...
    Examples::
        ...
        if labels is not None:
            ...
            #outputs = (loss,) + outputs
            #loss = self.instance_bce_with_logits(logits, labels)
            loss = self.debias_loss_fn(pooled_output, logits, bias, labels)

        return loss, logits #outputs  # (loss), reshaped_logits, (hidden_states), (attentions)
...
class LxmertForSequenceClassification(LxmertPreTrainedModel):
    ...
    def instance_bce(self, logits, labels):   
        assert logits.dim() == 2
    ...
    Examples::
        ...
        #print('/mnt/sda/erqing/VQA/CompressAndRobust/mask_train/hg_transformers/modeling_lxmert.py', [i.size for i in outputs])
        pooled_output = outputs[2]#.pooled_output
        #lang_output = output[0]
        #visual_output = output[1]
        ...
        if labels is not None:
            ...
            #loss = loss_fct(reshaped_logits, labels)
            #outputs = (loss,) + outputs
            loss = self.instance_bce(logits, labels)
...
class LxmertModel(LxmertPreTrainedModel):
    ...
    def forward(
        ...
        if input_ids is not None and inputs_embeds is not None:
            ...
            #print("/mnt/sda/erqing/VQA/CompressAndRobust/mask_train/hg_transformers/modeling_lxmert.pyinput_ids", input_ids)
            input_shape = input_ids.size()
