...
class ModuleUtilsMixin:
    ...
    def get_ffn_mask(self, ffn_mask: Tensor, num_hidden_layers: int) -> Tensor:
        """
        # Prepare ffn mask if needed
        # 1.0 in ffn_mask indicate we keep the head
        attention_probs has shape bsz x n_heads x N x N
        Arguments:
            ffn_mask: torch.Tensor or None: has shape [ffn_intermedia_dim] or [num_hidden_layers x ffn_intermedia_dim]
            num_hidden_layers: int
        Returns:
             Tensor of shape shape [num_hidden_layers x batch x ffn_intermedia_dim x seq_length x seq_length]
             or list with [None] for each layer
        """
        if ffn_mask is not None:
            ffn_mask = ffn_mask.unsqueeze(1).unsqueeze(1)
            assert ffn_mask.dim() == 4, f"ffn_mask.dim != 4, instead {ffn_mask.dim()}"
            ffn_mask = ffn_mask.to(dtype=self.dtype)  # switch to fload if need + fp16 compatibility
        else:
            ffn_mask = [None] * num_hidden_layers
...
class PreTrainedModel(nn.Module, ModuleUtilsMixin):
    ...
    def prune_ffns(self, ffns_to_prune: Dict):
        """ Prunes ffns of the base model.

        """
        self.base_model._prune_ffns(ffns_to_prune)
