...
class TrimCollator(DataCollator):
...
class DataCollatorForLanguageModeling(DataCollator):
    ...
    def _tensorize_batch(self, examples: List[torch.Tensor]) -> torch.Tensor:
        #只适用于做Mlm时候的切割
        #print('data_data_collator', examples)
        #print('data_data_collator_examples[0]', examples[0])
...
@dataclass
class DataCollatorForKnowledgeDistillation(DataCollator):
    """
    Data collator used for knowledge distillation.
    - collates batches of tensors, honoring their tokenizer's pad_token
    - preprocesses batches for knowledge distillation
    """
    ...
    def collate_batch(self, examples: List[torch.Tensor]) -> Dict[str, torch.Tensor]:
        batch = self._tensorize_batch(examples)
        inputs, _ = self.mask_tokens(batch)
