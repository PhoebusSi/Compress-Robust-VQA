            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                            ...
                            for key, value in results.items():
                                ...
                                if key=='eval_acc' or key=='eval_mcc' or key=='eval_pearson':
                                    ...
                                    if best_score<value:
                                        ...
                                        #if self.model_args.structured: #这个if是我自己加的！！
                                        ...
                                        results_at_best_score = results
                                        if self.model_args.structured:#这个if是我自己加的！！不加的话获取不到head_mask
                                            self.save_struc_model_mask(self.args.output_dir+'/best_eval_mask', head_mask, ffn_mask)
                    ...
                    if self.args.save_steps > 0 and self.global_step % self.args.save_steps == 0:
                        ...
                        if self.model_args.save_mlm_head:
                            if hasattr(model, 'cls'):
                                torch.save(model.cls.predictions, output_dir+'/mlm_head.bin')
                            if hasattr(model, 'lm_head'):
                                torch.save(model.lm_head, output_dir+'/mlm_head.bin')
