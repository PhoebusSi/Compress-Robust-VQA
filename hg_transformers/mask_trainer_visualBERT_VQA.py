...
class Trainer:
    ...
    def __init__(
        ...
        if self.args.training_type == 'Masker':   
            self.debiasing = self.args.Masker_type 
        else:
            self.debiasing = self.args.FT_type
        assert self.debiasing in ["normal", "lmh", "lpf", "rubi"]
    ...
    def reset_threshold(self, model, init_sparsity):
        ...
        for name, module in model.named_modules():
            #print('reset_threshold', name, module.requires_grad)
                                            ...
                                            if self.masker: 
                                                ...
                                                if isinstance(model,torch.nn.DataParallel):
                                                    ...
                                                    if hasattr(model, 'classifier'):
                                                        ...
                                                        torch.save(model.cls, self.args.output_dir+'/classifier4masker.bin')
                                                    else:
    ...
    def _training_step(
        ...
        outputs = model(input_ids=inputs[0].to(self.args.device), visual_embeds=inputs[1].to(self.args.device), labels=inputs[3].to(self.args.device))
        ...
        if self.args.training_type == 'Masker':
            if self.args.Masker_type == 'normal':
                ...
                # loss = self.debias_loss_fn(norm_pool_out, logits, bias, inputs[3], self.args.device)
    ...
    def _prediction_loop(
        ...
        for inputs in tqdm(dataloader, desc=description):
            ...
            with torch.no_grad():
                outputs = model(input_ids=inputs[0], visual_embeds=inputs[1], labels=inputs[3])
                if has_labels:
                    if self.debiasing == 'normal':
