...
# from .train_VQA_RobustMasker import HPmodel
...
def LPF_loss(logits, bias, max_label, device, gamma):
    # criterion=nn.CrossEntropyLoss()
    vqa_pt = F.softmax(logits, dim=-1)
    one_hot_labels = max_label.view(-1, 1)#[b, 1]
    small_value = torch.ones_like(vqa_pt).to(device=device) * 1.0e-7
    vqa_pt = torch.max(vqa_pt, small_value)
    qo_pt = torch.max(bias, small_value)
    ...
    lpf_loss =  (1 - feedback)**gamma * ce_loss
    lpf_loss = lpf_loss.mean()
    return lpf_loss


def RUBI_loss(logits, bias, max_label):
    criterion=nn.CrossEntropyLoss()
    fus_pred = logits * torch.sigmoid(bias) 
    loss = criterion(fus_pred, max_label)
    return loss
...
class Trainer:
    ...
    def __init__(
        ...
        self.masker = masker
        # self.masker_scheduler = masker.masker_scheduler
        self.lpf_loss = LPF_loss
        self.rubi_loss = RUBI_loss
        # for PoE
        # self.debias_loss_fn = BiasProduct().to(args.device) #LearnedMixin(0.36).to(args.device)
        ...
        assert self.args.training_type in ['Masker','FTonly', 'FTlmh', 'FTlpf', 'FTrubi','FT_trainedMask', 'FT_randMask']
        ...
        assert self.debiasing in ["normal", "lmh", "lpf", "rubi"]#add debiasing operation in Stage1 or Stage3
    ...
    def reset_threshold(self, model, init_sparsity):
        ...
        for name, module in model.named_modules():            
            if hasattr(module, 'threshold'):
                if 'module.' in name:
                    name = name[7:]
                modal = self.masker.name_in_module[name]
                new_sparsity =  self.masker.hpmodel.zerorate_dict[modal]
                # _num_zero_element = int(module.weight.nelement() * init_sparsity)
                _num_zero_element = int(module.weight.nelement() * new_sparsity) 
                if _num_zero_element == 0: #int(module.weight.nelement()):
                    _num_zero_element = 1
    ...
    def train(self, model_path: Optional[str] = None):
        ...
        # In stage2, “init_masker” completes model compression before model training, so it is reasonable that the performance tested here is relatively low. On the contrary, the test results after loading the model in Stage1 or Stage3 should be consistent with the performance of the loaded model.
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    if self.args.save_steps > 0 and self.global_step % self.args.save_steps == 0:
                        if self.args.evaluate_during_training:
                            results, eval_output = self.evaluate() #saving json file of predictions
                            for key, value in results.items():
                                ...
                                if key=='eval_acc':
                                    ...
                                    if best_score<value:
                                        ...
                                        if self.args.training_type == 'Masker':
                                        ...
                                        elif self.args.training_type == 'FTlpf':#saving lxmert(bce) or lxmert(lpf) models
                                            torch.save(model, self.args.output_dir+'/'+self.args.label4save+'_FTlpf_only.bin')#
                                        elif self.args.training_type == 'FTrubi':#saving lxmert(bce) or lxmert(rubi) models
                                            torch.save(model, self.args.output_dir+'/'+self.args.label4save+'_FTrubi_only.bin')#
                                            ...
                                            print("training_type should be in [Masker, FTonly, FTlmh, FTlpf, FTrubi, FT_trainedMask, FT_randMask]")
                                            assert 0==1
    ...
    def _training_step(
        ...
        if self.args.training_type == 'Masker':
            if self.args.Masker_type == 'normal':
                ...
                #for PoE
                # loss = self.debias_loss_fn(pool_out, logits, bias)
                # loss = self.debias_loss_fn(pool_out, logits, bias, inputs[3].to(self.args.device))
                # for LMH
                loss = self.debias_loss_fn(pool_out, logits, bias, inputs[3].to(self.args.device), self.args.device)
                score_input = EvalPrediction(logits, inputs[3])
            elif self.args.Masker_type == 'lpf':
                bias = inputs[6].to(self.args.device)
                loss = self.lpf_loss(logits, bias, inputs[7].to(self.args.device),self.args.device,self.args.gamma)
                score_input = EvalPrediction(logits, inputs[3])
            elif self.args.Masker_type == 'rubi':
                bias = inputs[6].to(self.args.device)
                loss = self.rubi_loss(logits, bias, inputs[7].to(self.args.device))
                score_input = EvalPrediction(logits, inputs[3])
        else:
            if self.debiasing == 'normal':
                ...
                bias = inputs[6].to(self.args.device)
                #for PoE
                # loss = self.debias_loss_fn(pool_out, logits, bias, self.args.device)
                # loss = self.debias_loss_fn(pool_out, logits, bias, inputs[3].to(self.args.device))
                #for LMH
                loss = self.debias_loss_fn(pool_out, logits, bias, inputs[3].to(self.args.device), self.args.device)
                score_input = EvalPrediction(logits, inputs[3])
            elif self.debiasing == 'lpf':
                bias = inputs[6].to(self.args.device)  
                loss = self.lpf_loss(logits, bias, inputs[7].to(self.args.device), self.args.device, self.args.gamma)
                score_input = EvalPrediction(logits, inputs[3])
            elif self.debiasing == 'rubi':
                bias = inputs[6].to(self.args.device) 
                loss = self.rubi_loss(logits, bias, inputs[7].to(self.args.device))
                score_input = EvalPrediction(logits, inputs[3])
    ...
    def save_model_mask(self, output_dir: Optional[str] = None):
        ...
        zero_sum4lang, elem_sum4lang =0., 0.
        zero_sum4vis, elem_sum4vis =0., 0.
        zero_sum4fus, elem_sum4fus =0., 0.
        zero_sum4p, elem_sum4p =0., 0.
        ...
        for name, module in self.model.named_modules():
            if hasattr(module, 'threshold'):
                ...
                modal = self.masker.name_in_module[name]
                if modal == "Lang":
                    zero_sum4lang  += (mask==0).sum()
                    elem_sum4lang += mask.numel()
                elif modal == "Vis":
                    zero_sum4vis  += (mask==0).sum()
                    elem_sum4vis += mask.numel()
                elif modal == "Fus":
                    zero_sum4fus  += (mask==0).sum()
                    elem_sum4fus += mask.numel()
                elif modal == "P":
                    zero_sum4p  += (mask==0).sum()
                    elem_sum4p += mask.numel()
                else:
                    assert 0==1 
        ...
        zero_rate_lang = 100*zero_sum4lang/elem_sum4lang 
        zero_rate_vis = 100*zero_sum4vis/elem_sum4vis  
        zero_rate_fus = 100*zero_sum4fus/elem_sum4fus  
        zero_rate_p = 100*zero_sum4p/elem_sum4p 
        logger.info("Saving model mask to %s", output_dir)
        logger.info("Zero rate = %.2f", zero_rate)
        logger.info("Zero rate LANG = %.2f", zero_rate_lang)
        logger.info("Zero rate VIS = %.2f", zero_rate_vis)
        logger.info("Zero rate FUS = %.2f", zero_rate_fus)
        logger.info("Zero rate P = %.2f", zero_rate_p)
            ...
            with torch.no_grad():
                ...
                if has_labels:
                    if self.debiasing == 'normal':
                        ...
                        #for PoE
                        # eval_losses += [self.debias_loss_fn(pool_out, logits, bias, self.args.device).mean().item()]
                        # eval_losses += [self.debias_loss_fn(pool_out, logits, bias, inputs[3].to(self.args.device)).mean().item()]
                        ...
                        step_eval_loss, logits, pool_out = outputs[:3]
                        bias = inputs[6].to(self.args.device)
                        eval_losses += [self.lpf_loss(logits, bias, inputs[7].to(self.args.device), self.args.device,self.args.gamma).mean().item()]
                    elif self.debiasing == 'rubi':
                        step_eval_loss, logits, pool_out = outputs[:3]
                        bias = inputs[6].to(self.args.device)
                        eval_losses += [self.rubi_loss(logits, bias, inputs[7].to(self.args.device)).mean().item()]
