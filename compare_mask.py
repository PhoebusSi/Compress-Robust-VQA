...
root_dir='/apdcephfs/share_47076/tmpv_xiuliu/LT/models/prun_bert/unstructured'
root_dir_imp = '/apdcephfs/share_47076/tmpv_xiuliu/bert-lt/pretrain_prun/imp_pretrain/wikitext-103'
...
output_dir = '/apdcephfs/share_47076/tmpv_xiuliu/LT/log/mask_dist/length512/%s'%sparsity
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

mask_dirs = {
        'mag': '/apdcephfs/share_47076/tmpv_xiuliu/bert-lt/pretrain_prun/oneshot/%s/mask.pt'%str(sparsity),
        }
...
#keys = ['mag'] + ['kd_seed%d'%s for s in range(1, 4)] + ['mlm_seed%d'%s for s in range(1, 4)] + ['imp_seed%d'%s for s in range(1, 4)]
def main():
